...
def main():
    ...
    for idx, _load in enumerate(load):
        ...
        if(result.l_mtu[-1] > sys.muscle.l_opt + sys.muscle.l_slack):
            v_ce[idx] = max(result.v_ce)
            force[idx] = result.tendon_force[-1]
        else:
            v_ce[idx] = min(result.v_ce)
            force[idx] = result.tendon_force[-1]
    plt.figure('Force-Velocity')
    plt.plot(v_ce, force/sys.muscle.f_max, '*')
    plt.plot(0.0, 1.0, 'ro', markersize='10')
    plt.text(0.05, 1.05, 'Isometric \n contraction',
             fontsize=14)
    plt.plot(np.zeros((25, 1)),
             np.linspace(0., 2., 25), '--r')
    plt.text(-0.75, 0.8, 'Eccentric \n contraction',
             fontsize=14)
    plt.arrow(-0.25, 0.7, -0.5, 0.)
    plt.text(0.75, 0.8, 'Concentric \n contraction',
             fontsize=14)
    plt.arrow(0.75, 0.7, 0.5, 0.)
