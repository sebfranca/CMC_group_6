...
class MuscleResult(NamedTuple):
    """ MuscleResult helps to save the results of muscle simulation.

    All the attributes of the class are numpy ndarrays. These attribute
    store the respective values for each timestep of the simulation.
    The length of the arrays(of each attribute) would be equal to the
    no of iterations run. This class has no method as it is being used
    to store the data from the simulation.

    Attributes:
    time: simulation time of each iteration
    activation: activation at each iteration
    l_ce: length of CE element at each iteration
    l_mtu: length MTU at each iteration
    v_ce: veclocity of CE element length at each iteration
    active_forve: actively applied forces at each iteration
    passive_force: passive forces  at each iteration
    tendon_force: tendon forces  at each iteration
    """
    time: NDArray[(Any,), float]
    activation: NDArray[(Any,), float]
    l_ce: NDArray[(Any,), float]
    l_mtu: NDArray[(Any,), float]
    v_ce: NDArray[(Any,), float]
    active_force: NDArray[(Any,), float]
    passive_force: NDArray[(Any,), float]
    tendon_force: NDArray[(Any,), float]
...
class Muscle(object):
    ...
    def __init__(self, parameters: MuscleParameters):
        ...
        self.parameters: MuscleParameters = parameters

        # Internal variables
        self._l_ce: float = 0.0
        self._v_ce: float = 0.0

        # Muscle specific parameters initialization
        self.l_slack: float = parameters.l_slack
        self.l_opt: float = parameters.l_opt
        self.v_max: float = parameters.v_max
        self.f_max: float = parameters.f_max
        self.pennation: float = parameters.pennation
    ...
    def active_force(self):
        ...
        return self.compute_active_force(self.l_ce, self.v_ce, self._activation)
